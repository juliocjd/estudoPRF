...
import { FSRS_CONFIG, DERIVED_CE_ID_START } from '/tmp/study-config-full.mjs';
...
async function routeRequest(request, response) {
  ...
  if (url.pathname === '/api/contran-prf-2021/map' && request.method === 'GET') {
    sendJson(response, 200, getContranPrf2021Map());
    return;
  }
  ...
  if (url.pathname === '/api/cebraspe-risk-report' && request.method === 'GET') {
    sendJson(response, 200, getCebraspeRiskReport(url.searchParams));
    return;
  }

  if (url.pathname === '/api/points-optimizer' && request.method === 'GET') {
    sendJson(response, 200, getPointsOptimizer(url.searchParams));
    return;
  }
  ...
  if (url.pathname === '/api/exam-simulations' && request.method === 'GET') {
    sendJson(response, 200, listExamSimulations());
    return;
  }

  const simulationStateMatch = url.pathname.match(/^\/api\/exam-simulations\/([^/]+)$/);
  if (simulationStateMatch && request.method === 'GET') {
    sendJson(response, 200, getExamSimulationState(simulationStateMatch[1]));
    return;
  }
...
function initGranCursosLessonProgressSchema(database, client = activeDbClient) {
  if (client === 'postgres') {
    database.exec(`
...
function updateGranCursosLessonProgress(body = {}) {
  ...
  db.prepare(`
    ...
    VALUES ('local', ?, ?, CASE WHEN ? = 1 THEN CURRENT_TIMESTAMP ELSE NULL END, 'gran_cursos_transito_prf', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  `).run(lessonNumber, watchedValue, watchedValue);
...
function updateGranCursosLessonProgressBulk(body = {}) {
  ...
  db.exec('BEGIN');
  try {
    for (const lessonNumber of lessonNumbers) upsert.run(lessonNumber, watchedValue, watchedValue);
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
    throw error;
...
function getContranPrf2021Map() {
  if (!hasContranPrf2021MapTable()) {
    return { available: false, reason: 'Mapa CONTRAN PRF 2021 ainda nao importado.', total: 0, rows: [] };
  }
  const rows = db.prepare(`
    SELECT
      m.source_organ,
    ...
    ORDER BY
      m.source_year::int,
      regexp_replace(m.source_number, '\\.', '', 'g')::int,
      m.source_number
  `).all();
...
function getContranPrf2021LegalAliasMap() {
  const aliases = db.prepare(`
    SELECT
      current_organ,
      current_number::text AS current_number_text,
      current_year::text AS current_year_text,
      old_number::text AS old_number_text,
      old_year::text AS old_year_text
    FROM legal_norm_aliases
    WHERE alias_reason = 'PRF_2021_current_resolution_map'
    ORDER BY old_year, old_number
  `).all();
  const map = new Map();
  aliases.forEach((row) => {
    const key = `${row.current_organ || ''}:${row.current_number_text || ''}/${row.current_year_text || ''}`;
    if (!map.has(key)) map.set(key, []);
    map.get(key).push(`${row.old_number_text}/${row.old_year_text}`);
  });
  return map;
}
...
function startExamSimulation(body) {
  ...
  // Prova Cebraspe é 100% certo/errado: por padrão o simulado usa só itens C/E.
  ...
  if (certoErradoOnly && plan.questionIds.length) {
...
function finishExamSimulation(simulationId) {
  ...
  const totalItems = Number(simulation.total_items || 0) || 120;
  const quarterSize = Math.max(1, Math.ceil(totalItems / 4));
  const fatigue = db.prepare(`
    SELECT CAST((position - 1) / ${quarterSize} AS INTEGER) + 1 AS quarter,
      COUNT(*) AS answered,
      SUM(CASE WHEN is_correct = 1 THEN 1 ELSE 0 END) AS correct,
      SUM(CASE WHEN is_correct = 0 THEN 1 ELSE 0 END) AS wrong,
      SUM(CASE WHEN COALESCE(answer_letter, '') = '' THEN 1 ELSE 0 END) AS blank,
      COALESCE(SUM(score), 0) AS score,
      COALESCE(AVG(NULLIF(elapsed_ms, 0)), 0) AS avg_elapsed_ms
    FROM exam_simulation_items
    WHERE simulation_id = ?
    GROUP BY CAST((position - 1) / ${quarterSize} AS INTEGER) + 1
    ORDER BY quarter
  `).all(simulationId);

  // Contrafactual de política de marcação: quanto valeria a prova
  // se itens de cada nível de confiança tivessem ficado em branco.
  const byConfidence = new Map(confidence.map((row) => [row.confidence, row]));
  const scoreOf = (row) => Number(row?.score || 0);
  const actualScore = Number(aggregate.score_total || 0);
  const markingPolicies = [
    { policy: 'como_marcado', label: 'Como você marcou', score: actualScore },
    { policy: 'guess_em_branco', label: 'Chutes (guess) em branco', score: actualScore - scoreOf(byConfidence.get('guess')) },
  ...
  return {
    ...
    fatigue,
    markingPolicies,
    bestMarkingPolicy: markingPolicies[0]?.policy || 'como_marcado',
    dangerousSubjects,
...
function listExamSimulations() {
  const rows = db.prepare(`
    SELECT id, profile_id, mode, total_items, started_at, finished_at,
      score_total, score_block_1, score_block_2, score_block_3,
      blank_count, correct_count, wrong_count, passed_cutoffs
...
function getExamSimulationState(simulationId) {
  const simulation = db.prepare('SELECT * FROM exam_simulations WHERE id = ?').get(simulationId);
  if (!simulation) {
    return { error: 'Simulado nao encontrado.' };
  }
  const items = db.prepare(`
    SELECT question_id, position, block_key, subject_key,
      answer_letter, is_correct, score, confidence, elapsed_ms
    FROM exam_simulation_items
    WHERE simulation_id = ?
    ORDER BY position
  `).all(simulationId);
  const answeredCount = items.filter((item) => item.answer_letter !== null && item.answer_letter !== undefined).length;
  return {
    simulation,
    items,
    answeredCount,
    finished: Boolean(simulation.finished_at)
  };
}

/**
 * Otimizador pontos-por-hora: cruza peso da matéria na prova × desempenho atual
 * → onde cada hora de estudo compra mais pontos, e nota projetada vs corte 2021.
 */
function getPointsOptimizer(searchParams) {
  const profileId = resolveProfileId(searchParams?.get?.('profile'));
  const TARGET_ACCURACY = 0.95;
  const weights = db.prepare(`
    SELECT subject_key, subject_label, block_key, block_label, expected_items, expected_pct
    FROM exam_subject_weights
    WHERE profile_id = ?
    ORDER BY COALESCE(expected_items, 0) DESC
  `).all(profileId);

  const performance = db.prepare(`
    SELECT qes.subject_key,
      SUM(CASE WHEN sa.is_correct = 1 THEN 1 ELSE 0 END) AS correct,
      SUM(CASE WHEN sa.is_correct = 0 THEN 1 ELSE 0 END) AS wrong
    FROM study_answers sa
    JOIN question_exam_subjects qes
      ON qes.question_id = sa.question_id
      AND qes.profile_id = ?
    WHERE sa.is_correct IS NOT NULL
    GROUP BY qes.subject_key
  `).all(profileId);
  const performanceBySubject = new Map(performance.map((row) => [row.subject_key, row]));

  const subjects = weights.map((row) => {
    const perf = performanceBySubject.get(row.subject_key) || { correct: 0, wrong: 0 };
    const correct = Number(perf.correct || 0);
    const wrong = Number(perf.wrong || 0);
    const sampleSize = correct + wrong;
    // Suavização bayesiana (prior 0.5) para não superreagir a amostras pequenas.
    const accuracy = (correct + 2) / (sampleSize + 4);
    const expectedItems = Number(row.expected_items || 0)
      || (Number(row.expected_pct || 0) / 100) * 120;
    // Cebraspe C/E: marcar tudo com acurácia p rende (2p-1) por item;
    // se EV negativo, política ótima é deixar em branco (0).
    const evPerItem = 2 * accuracy - 1;
    const expectedPoints = expectedItems * Math.max(0, evPerItem);
    const potentialGain = expectedItems * Math.max(0, TARGET_ACCURACY - accuracy) * 2;
    return {
      subjectKey: row.subject_key,
      subjectLabel: row.subject_label,
      blockKey: row.block_key,
      blockLabel: row.block_label,
      expectedItems: Number(expectedItems.toFixed(1)),
      sampleSize,
      accuracy: Number(accuracy.toFixed(3)),
      evPerItem: Number(evPerItem.toFixed(3)),
      markingAdvice: evPerItem > 0.05 ? 'marcar' : evPerItem < -0.05 ? 'em_branco' : 'zona_neutra',
      expectedPoints: Number(expectedPoints.toFixed(1)),
      potentialGain: Number(potentialGain.toFixed(1)),
      lowData: sampleSize < 10
    };
  }).sort((left, right) => right.potentialGain - left.potentialGain);

  const projectedScore = subjects.reduce((sum, subject) => sum + subject.expectedPoints, 0);
  const blockTotals = {};
  for (const subject of subjects) {
    const key = subject.blockKey || 'sem_bloco';
    blockTotals[key] = (blockTotals[key] || 0) + subject.expectedPoints;
  }
  ...
  return {
    ...
    reference2021: {
      lastCallScore: 73,
      gapToLastCall: Number((projectedScore - 73).toFixed(1))
    },
    blockTotals: Object.fromEntries(
      Object.entries(blockTotals).map(([key, value]) => [key, Number(value.toFixed(1))])
    ),
    cutoffs: { bloco_1: 15, bloco_2: 10, bloco_3: 10, total: 50 },
    subjects,
...
function saveLawClozeAnswer(cardId, body) {
  ...
  const fsrs = fsrsScheduleReview(state, rating, now, {
...
const CONFIDENCE_CLAIMED_PROBABILITY = { sure: 0.9, doubt: 0.7, guess: 0.55 };

function getCebraspeRiskReport(searchParams) {
  const scope = searchParams?.get?.('scope') === 'all' ? 'all' : 'certo_errado';
  const typeFilter = scope === 'all' ? '' : "AND q.type_question = 'CERTO_ERRADO'";

  const rows = db.prepare(`
    SELECT COALESCE(NULLIF(sa.confidence, ''), 'sem_confianca') AS confidence,
      SUM(CASE WHEN sa.is_correct = 1 THEN 1 ELSE 0 END) AS correct,
      SUM(CASE WHEN sa.is_correct = 0 THEN 1 ELSE 0 END) AS wrong,
      SUM(CASE WHEN sa.is_correct IS NULL THEN 1 ELSE 0 END) AS unknown,
      SUM(CASE WHEN sa.is_correct = 1 THEN 1 WHEN sa.is_correct = 0 THEN -1 ELSE 0 END) AS score
    FROM study_answers sa
    JOIN questions q ON q.id_question = sa.question_id
    WHERE 1 = 1 ${typeFilter}
    GROUP BY COALESCE(NULLIF(sa.confidence, ''), 'sem_confianca')
    ORDER BY confidence
  `).all();

  const byMateria = db.prepare(`
    SELECT q.materia,
      COALESCE(NULLIF(sa.confidence, ''), 'sem_confianca') AS confidence,
      SUM(CASE WHEN sa.is_correct = 1 THEN 1 ELSE 0 END) AS correct,
      SUM(CASE WHEN sa.is_correct = 0 THEN 1 ELSE 0 END) AS wrong
    FROM study_answers sa
    JOIN questions q ON q.id_question = sa.question_id
    WHERE sa.is_correct IS NOT NULL ${typeFilter}
    GROUP BY q.materia, COALESCE(NULLIF(sa.confidence, ''), 'sem_confianca')
    HAVING COUNT(*) >= 3
    ORDER BY q.materia, confidence
  `).all();

  // Calibração: acurácia real vs probabilidade declarada + Brier score por nível.
  const calibration = rows
    .filter((row) => CONFIDENCE_CLAIMED_PROBABILITY[row.confidence])
    .map((row) => {
      const claimed = CONFIDENCE_CLAIMED_PROBABILITY[row.confidence];
      const total = Number(row.correct || 0) + Number(row.wrong || 0);
      const actual = total > 0 ? Number(row.correct || 0) / total : null;
      const brier = total > 0
        ? (((1 - claimed) ** 2) * Number(row.correct || 0) + (claimed ** 2) * Number(row.wrong || 0)) / total
        : null;
      return {
        confidence: row.confidence,
        claimedProbability: claimed,
        actualAccuracy: actual === null ? null : Number(actual.toFixed(3)),
        gap: actual === null ? null : Number((actual - claimed).toFixed(3)),
        brierScore: brier === null ? null : Number(brier.toFixed(4)),
        sampleSize: total
      };
    });

  // Política de marcação por matéria × confiança (EV no scoring ±1).
  const markingPolicy = {};
  for (const row of byMateria) {
    const total = Number(row.correct || 0) + Number(row.wrong || 0);
    if (!total) continue;
    const accuracy = Number(row.correct || 0) / total;
    const ev = 2 * accuracy - 1;
    if (!markingPolicy[row.materia]) markingPolicy[row.materia] = [];
    markingPolicy[row.materia].push({
      confidence: row.confidence,
      accuracy: Number(accuracy.toFixed(3)),
      evPerItem: Number(ev.toFixed(3)),
      advice: ev > 0.05 ? 'marcar' : ev < -0.05 ? 'em_branco' : 'zona_neutra',
      sampleSize: total
    });
  }
...
function updateQuestionMastery(database, question, answerResult, attemptMeta) {
  ...
  // Agendamento FSRS: substitui os intervalos fixos legados (7→15→30→60).
  const fsrsState = Number(previous.fsrs_stability || 0) > 0 ? {
    stability: Number(previous.fsrs_stability),
    difficulty: Number(previous.fsrs_difficulty),
    reps: Number(previous.fsrs_reps || 0),
    lapses: Number(previous.fsrs_lapses || 0),
    lastReviewAt: previous.fsrs_last_review || previous.last_seen_at || null
  } : null;
  const fsrsRating = fsrsGradeAnswer({
    isCorrect,
    confidence: attemptMeta.confidence,
  ...
  database.prepare(`
    INSERT INTO question_mastery (
      ...
      fsrs_stability, fsrs_difficulty, fsrs_reps, fsrs_lapses, fsrs_last_review,
      fsrs_retrievability, fsrs_version
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(question_id) DO UPDATE SET
      attempts = excluded.attempts,
  ...
  `).run(
    ...
    stability,
    nowSql,
    fsrs ? fsrs.stability : (previous.fsrs_stability || null),
    fsrs ? fsrs.difficulty : (previous.fsrs_difficulty || null),
    fsrs ? fsrs.reps : (previous.fsrs_reps || 0),
    fsrs ? fsrs.lapses : (previous.fsrs_lapses || 0),
    fsrs ? nowSql : (previous.fsrs_last_review || null),
    fsrs ? fsrs.retrievability : null,
    fsrs ? fsrs.version : (previous.fsrs_version || null)
  );
...
function initStudySchema(database) {
  ...
  ensureColumn(database, 'question_mastery', 'fsrs_stability', 'REAL');
  ensureColumn(database, 'question_mastery', 'fsrs_difficulty', 'REAL');
  ensureColumn(database, 'question_mastery', 'fsrs_reps', 'INTEGER DEFAULT 0');
  ensureColumn(database, 'question_mastery', 'fsrs_lapses', 'INTEGER DEFAULT 0');
  ensureColumn(database, 'question_mastery', 'fsrs_last_review', 'TEXT');
  ensureColumn(database, 'question_mastery', 'fsrs_retrievability', 'REAL');
  ensureColumn(database, 'question_mastery', 'fsrs_version', 'TEXT');
