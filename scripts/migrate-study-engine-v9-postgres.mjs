...
const MIGRATION_PATH = path.join(ROOT_DIR, 'migrations', '20260603_study_engine_v9_scoring_and_clusters.postgres.sql');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!databaseUrl && !dryRun) {
    console.error('Defina DATABASE_URL ou passe --database-url para aplicar a migration v9 no Postgres.');
    process.exit(1);
  }
  await migrateStudyEngineV9Postgres({ databaseUrl, dryRun });
}

export async function migrateStudyEngineV9Postgres({ databaseUrl, dryRun = false }) {
  ...
  try {
    ...
    console.log('Migration aplicada: study engine v9.');
  } finally {
